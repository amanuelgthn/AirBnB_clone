...
class FileStorage_testcase(unittest.TestCase):
    ...
    def test_file_path(self):
        my_model = BaseModel(id='b6a6e15c-c67d-4312-9a75-9d084935e579',
        ...
        for obj_id in all_objs.keys():
            if obj_id == "id":
                self.assertEqual(all_objs[id],
                                 'b6a6e15c-c67d-4312-9a75-9d084935e579')
            if obj_id == "created_at":
                self.assertEqual(all_objs[created_at],
                                 datetime(2017, 9, 28, 21, 5, 54, 119572))
            if obj_id == "updated_at":
                self.assertEqual(all_objs[updated_at],
                                 datetime(2017, 9, 28, 21, 5, 54, 119572))
